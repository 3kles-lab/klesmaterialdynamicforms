        ...
        <div class="selection-list" [formGroup]="group">
            ...
            <mat-selection-list [attr.id]="field.id" [multiple]="field.multiple" [ngClass]="ngClass()" (selectionChange)="onSelectionChange($event)">
                ...
                <cdk-virtual-scroll-viewport [itemSize]="field.itemSize || 20" style="height:100%">
                    ...
                    <mat-list-option *cdkVirtualFor="let item of options; templateCacheSize: 0" [value]="item" [selected]="selection.isSelected(item)">
                ...
                } @else{ @if (!field.autocompleteComponent) { @for (item of options; track item) {
                <mat-list-option [value]="item" [selected]="selection.isSelected(item)">
                    {{ (field.property ? item[field.property] : item) | klesTransform : field.pipeTransform }}
                </mat-list-option>
                } } @else { @for (item of options; track item) {
                <mat-list-option [value]="item" [selected]="selection.isSelected(item)">
                    <ng-container klesComponent [component]="field.autocompleteComponent" [value]="item" [field]="field"></ng-container>
                </mat-list-option>
                } } } }@else{
...
export class KlesFormSelectionListSearchComponent extends KlesFieldAbstract implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.selection = new KlesSelectionModel<any>(this.field.multiple || false, [], true, (o1: any, o2: any) => {
                    ...
                    return (this.field.options instanceof Observable ? this.options$ : of(this.field.options)).pipe(
                        map((options) => {
                            ...
                            return options.filter((option) => {
                                return (
                                    (!this.field.property && option?.toString().toLowerCase().includes(value)) ||
