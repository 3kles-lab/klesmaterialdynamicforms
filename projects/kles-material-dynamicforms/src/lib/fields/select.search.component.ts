...
import { ChangeDetectorRef, Component, inject, OnDestroy, OnInit, QueryList, signal, ViewChild, ViewChildren } from '@angular/core';
...
export class KlesFormSelectSearchComponent extends KlesFieldAbstract implements OnInit, OnDestroy {
            ...
            .pipe(
                ...
                switchMap((value) => {
                    return concat(
                        ...
                        this.onSearchChange(value).pipe(
                            take(1),
    ...
    protected onOpenChange(isOpen: boolean): Observable<any[]> {
        if (isOpen) {
            ...
            return of(
                this.group.controls[this.field.name].value !== undefined && this.group.controls[this.field.name].value !== null
                ...
                map((options) => {
                    return options.filter((option) => {
                        if (this.field.searchKeys && this.field.searchKeys.length) {
                            return (
                                this.field.searchKeys.some((searchKey) => {
                                    if (option[searchKey]) {
